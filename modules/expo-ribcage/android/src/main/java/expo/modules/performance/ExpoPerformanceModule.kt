...
class ExpoPerformanceModule : Module() {
    ...
    private var monitoringInterval: Long = 1000L
    ...
    override fun definition() = ModuleDefinition {
        ...
        Events("memoryUpdate", "cpuUpdate")
        
        // Native monitoring functions
        Function("startMonitoring") { intervalMs: Int ->
    ...
    private fun startMonitoring(intervalMs: Int) {
        // Input validation
        val validInterval = intervalMs.toLong().coerceIn(MIN_MONITORING_INTERVAL, MAX_MONITORING_INTERVAL)
        ...
        monitoringInterval = validInterval
        isMonitoring.set(true)
        isPaused.set(false)
        
        createMonitoringRunnable()
    ...
    private fun resumeMonitoring() {
        if (isMonitoring.get() && isPaused.get()) {
            isPaused.set(false)
            monitoringRunnable?.let { runnable ->
                monitoringHandler.post(runnable)
            }
        }
    }
    
    private fun getMonitoringState(): Map<String, Any> {
        return mapOf(
            "isMonitoring" to isMonitoring.get(),
            "intervalMs" to monitoringInterval.toInt(),
            "isPaused" to isPaused.get()
        )
    }
    
    private fun createMonitoringRunnable() {
        monitoringRunnable = object : Runnable {
            override fun run() {
                if (isMonitoring.get() && !isPaused.get()) {
                    // Emit memory update
                    val memoryMetrics = MemoryMetrics().apply {
                        ramUsageInMB = getMemoryUsage()
                        timestamp = System.currentTimeMillis().toDouble()
                    }
                    sendEvent("memoryUpdate", memoryMetrics)
                    
                    // Emit CPU update
                    val cpuMetrics = CPUMetrics().apply {
                        cpuUsage = getCPUUsage()
                        timestamp = System.currentTimeMillis().toDouble()
                    }
                    sendEvent("cpuUpdate", cpuMetrics)
