...
import { FPSClient } from './fps.client';
...
export class ExpoPerformanceClient {
  ...
  public readonly fps: FPSClient;

  constructor() {
    this.memory = new MemoryClient();
    this.cpu = new CPUClient();
    this.fps = new FPSClient();
  }
  ...
  startMonitoring(options: MonitoringOptions): void {
    ...
    this.fps.startMonitoring(options);
  }
  ...
  isMonitoring(): boolean {
    return this.memory.isMonitoring() || this.cpu.isMonitoring() || this.fps.isMonitoring();
  }
   ...
   * @param durationMs Duration in milliseconds (1000-30000)
   */
  stressTestMemory(durationMs: number = 10000): void {
    try {
      const validDuration = Math.max(1000, Math.min(30000, durationMs));
      ExpoPerformance.stressTestMemory(validDuration);
      console.log(`Memory stress test started for ${validDuration}ms`);
    } catch (error) {
      console.error('Failed to start memory stress test:', error);
    }
  }
   ...
   * @param durationMs Duration in milliseconds (1000-30000)
   */
  stressTestCPU(durationMs: number = 10000): void {
    try {
      const validDuration = Math.max(1000, Math.min(30000, durationMs));
      ExpoPerformance.stressTestCPU(validDuration);
      console.log(`CPU stress test started for ${validDuration}ms`);
    } catch (error) {
      console.error('Failed to start CPU stress test:', error);
    }
  }

  /**
   * Start both memory and CPU stress tests simultaneously
   * @param durationMs Duration in milliseconds (1000-30000)
   */
  stressTestBoth(durationMs: number = 10000): void {
    this.stressTestMemory(durationMs);
    // Delay CPU stress test slightly to see separate effects
    setTimeout(() => {
      this.stressTestCPU(durationMs);
    }, 1000);
  }
